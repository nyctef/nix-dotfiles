...
### Phase B — network: flip policy L3/L4 → L7 (biggest correctness win)  ✅ scaffolded (untested)
...
- Enforcement is **transparent** (iptables `REDIRECT` + uid-owner split: agent
  uid → proxy, proxy uid → free) so it can't be bypassed even if the app
  ignores `HTTP_PROXY`.
...
- Self-verifying: asserts a blocked host fails AND an allowed host succeeds.
...
- **Caveat (carry forward):** containers the agent spawns via the inner dockerd
  get their own netns → not yet forced through the egress controls. Under sysbox
  they share the same container network, but a determined agent could configure
  its inner containers to bypass. Fixing this requires configuring the inner
  dockerd's default network or iptables FORWARD rules.
...
## Current status: Phase A proven, Phase B scaffolded
...
- ✅ Phase B scaffolded: mitmproxy transparent proxy + iptables mandatory
  floor + hostname allowlist + domain fronting rejection + self-verification

Next step: **test Phase B end to end** on `tachikoma`, then **Phase C** —
credential injection (keep secrets off the agent).
