...
if command -v git &>/dev/null; then
    if git ls-remote https://github.com/nyctef/nix-dotfiles HEAD >/dev/null 2>&1; then
        pass "git ls-remote (fetch/clone read allowed)"
    else
        fail "git ls-remote failed (fetch read should be allowed)"
    fi
else
    skip "git ls-remote — git not available"
fi

# THE write test: opening an issue must be blocked by the policy *before* it
# reaches GitHub. If this ever returns 2xx, a real issue was created — the
# helper fails loudly in that case.
expect_github_write_blocked "open issue (write blocked)" \
    "https://api.github.com/repos/nyctef/nix-dotfiles/issues" POST \
    -H 'Authorization: token SANDBOX-PLACEHOLDER-GH-TOKEN' \
    -H 'Content-Type: application/json' \
    -d '{"title":"sandbox-egress-test (should never be created)"}'

# Gist creation — a write, denied.
expect_github_write_blocked "create gist (write blocked)" \
