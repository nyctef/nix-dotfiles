function prompt_login --description "display user name for the prompt"
    ...
    # If we're running via SSH, add the hostname
    set prompt_host
    if set -q SSH_TTY
        set prompt_host (string join '' '@' (set_color $fish_color_host_remote) (prompt_hostname))
    end

    echo -n -s (set_color $fish_color_user) "$USER" (set_color normal) "$prompt_host" (set_color normal)
end
